			    ...
			    if($bio->in_out_mode == 0) {
				  ...
				  if($valid) {
					...
					if(count($attendance_data)==0) {
				      ...
				      $obt_data = $obt_res->row();
					  
					  if(count($obt_data)) {
					    $obt_from = new DateTime($obt_data->full_timein);
						$obt_to = new DateTime($obt_data->full_timeout);
						if($obt_to->format("U") < $shift_out->format("U")) {
						  $undertime = ($shift_out->format("U")-$obt_to->format("U"))/60;
						}
						else {
						  $undertime = 0;
						}
