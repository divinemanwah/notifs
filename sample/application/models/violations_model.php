	...
	public function add($type, $data, $details = null) {
		...
		switch($type) {
			...
			case 2:
                ...
                if(isset($data['initial']) && is_numeric($data['initial']))
                    $emp_data['initial'] = intval($data['initial'], 10);
			
				$ret = !$dup &&
						$this->db->insert('hr_member_violations', $emp_data);
				
				if($ret && !isset($emp_data['initial'])) {
