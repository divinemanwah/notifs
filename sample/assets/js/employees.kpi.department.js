$(function () {
    ...
    var kpiDeptDisplayView = $('table#kpi-department-table');
    var kpiDeptScoringView = $('div#kpi-scoring-table');
    ...
    var kpiDeptScoringTbl = kpiDeptScoringView.handsontable({
        ...
        afterChange: function (changes, source) {
            ...
            if (changes && source !== 'loadData') {
                ...
                $.ajaxq('kpideptscorequeue', {
                    url: base_url + 'employees/updateDeptKpiScore',
                    dataType: 'json',
                    type: 'POST',
                    data: _data
                }).done();
        ...
        beforeChange: function (changes, source) {
            ...
            if (changes[0][3] > base_score) {
                ...
                //  alertNotif(errTitle, "Score should not be zero or null");
                return false;
            } else {
                return true;
            }
    ...
    function loadPagination(total_count, limit_default, current_page, pagination_limit) {
        ...
        var pagination_lbl = '<ul class="pagination page-green">';
...
//        $("#tk-hrmis-pager-top").html(pagination_lbl);
    ...
    function setScoringView(page_num) {
        ...
        kpiDeptScoringView.addClass('hidden');
        kpiDeptDisplayView.addClass('hidden');
        ...
        $.getJSON(
                base_url + 'employees/getDeptScoreData'
                            + "/" + $('button#kpi-filter-nation-btn').data('id')
                            + "/" + $('button#kpi-filter-dept-btn').data('id') 
                            + "/" + page_num
                            + "/" + limit_default
                            + "/" + $('input#kpi-display-inclusive-year').val() + "-" + $('input#kpi-display-inclusive-month').val(),
...
//                    kpiScoringPager.addClass('hidden');
                    noRecordScreen.addClass('hidden');
                    loadingScreen.addClass('hidden');
                    kpiDeptScoringView.removeClass('hidden');
                    
                    
                    kpiDeptScoringTbl.handsontable('getInstance').loadData(data.data);
                    loadPagination(data.total_count, limit_default, page_num, 9);
                }
        );

        kpiDeptScoringView.show();
        loadingScreen.addClass('hidden');
...
//        noRecordScreen.addClass('hidden');
        ...
        $.ajax({
            url: base_url + "employees/getDepartmentScores",
            data: {
                limit: limit_default,
            ...
            success: function (data) {
                ...
                noRecordScreen.addClass('hidden');
                loadingScreen.addClass('hidden');
                kpiDeptDisplayView.removeClass('hidden');
                kpiScoringPager.removeClass('hidden');
                
                kpiDeptDisplayView.handsontable({
