$(function () {
    var processing = false;
	  ...
	  serverSide: true,
	  ajax: {
		  url: base_url + 'obt/getAllForApproval',
		  type: "POST",
		  data: function(d) {
	  ...
	  columns: [
		  ...
		  render: function (d,t,r,m) {
		    return '\
			  <div class="hidden-sm hidden-xs action-buttons">\
			    ...
			    '+((r.status==1 && r.approved_level == r.user_level)?'<a class="green obt-approve" href="" title="Approve"><i class="ace-icon fa fa-check bigger-130"></i></a>':'')+'\
				'+((r.status==1 && r.approved_level == r.user_level)?'<a class="red obt-reject" href="#" title="Reject"><i class="ace-icon fa fa-remove bigger-130"></i></a>':'')+'\
			  </div>\
			  <div class="hidden-md hidden-lg">\
				<div class="inline position-relative">\
				  ...
				  <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">\
				    ...
				    <li>\
					  <a href="#" class="tooltip-success obt-approve" data-rel="tooltip" title="Approve">\
					    <span class="green">\
					...
					'+((r.status==1 && r.approved_level == r.user_level)?'\
					<li>\
					  <a href="#" class="tooltip-error obt-reject" data-rel="tooltip" title="Reject">\
						<span class="red">\
	    ...
	    rowCallback: function (r, d) {
		  ...
		  $('a.request-view', r).click(function (e) {
			...
			$("#obt-requester").html("");
			$("#obt-approve").removeData().hide();
			$("#obt-reject").removeData().hide();
			
			$.post(
			  base_url + "obt/getOBTApplication",
			  {request_id: d["obt_app_id"]},
			  function(response){
				$("#request-id").val(response.data[0].obt_app_id);
				$("#obt-date").datepicker("setDate",response.data[0].date);
				$("#obt-time-from").val(response.data[0].time_in);
				$("#obt-time-to").val(response.data[0].time_out);
				$("#reason").val(response.data[0].reason);
				$("#obt-requester").html(d["sender"]);
				if(response.data[0].status == 1 && d["approved_level"] == d["user_level"]) {
				  $("#obt-approve").data({level: d["approved_level"], id: d["approval_id"], grp_id: d["obt_apprv_grp_id"], app_id: d["obt_app_id"], label: d["sender"] + " OBT on ["+d["date"]+"] from "+d["time_in"]+" to "+d["time_out"], action: "approve"}).show();
				  $("#obt-reject").data({id: d["approval_id"], grp_id: d["obt_apprv_grp_id"], app_id: d["obt_app_id"], label: d["sender"] + " OBT on ["+d["date"]+"] from "+d["time_in"]+" to "+d["time_out"], action: "reject"}).show();
				}
		  ...
		  $('a.obt-approve', r).click(function (e) {
			e.preventDefault();
			$('button.modal-action-btn',"div#approval-modal").data({level: d["approved_level"], id: d["approval_id"], grp_id: d["obt_apprv_grp_id"], app_id: d["obt_app_id"], label: "["+d["date"]+"] "+d["time_in"]+"-"+d["time_out"], action: "approve"});
			$("p#remarks_lbl","div#approval-modal").hide();
			$('textarea#obt-remarks',"div#approval-modal").hide();
			$('div#approval-modal').modal('show');
		  });
		  $('a.obt-reject', r).click(function (e) {
			e.preventDefault();
			$('button.modal-action-btn',"div#approval-modal").data({id: d["approval_id"], grp_id: d["obt_apprv_grp_id"], app_id: d["obt_app_id"], label: "["+d["date"]+"] "+d["time_in"]+"-"+d["time_out"], action: "reject"});
			$("p#remarks_lbl","div#approval-modal").show();
			$('textarea#obt-remarks',"div#approval-modal").show();
			$('div#approval-modal').modal('show');
		  });
	    }
	  }),
	...
	$('button.modal-action-btn',"div#approval-modal").off("click").click(function(e) {
	  ...
	  if($(this).data("action") == "reject") {
	    $.post(
	      base_url + "obt/rejectOBT",
	      {obt_app_id: $('button.modal-action-btn',"div#approval-modal").data("app_id"), remarks: $('textarea#obt-remarks',"div#approval-modal").val()},
	      function(response) {
		    processing = false;
	    ...
	    $.post(
	      base_url + "obt/approveOBT",
	      {
		    obt_app_id: $('button.modal-action-btn',"div#approval-modal").data("app_id"),
	...
	$("#obt-approve").off("click").click(function(e){
	  e.preventDefault();
	  $('div#request-modal').modal('hide');
	  $('button.modal-action-btn',"div#approval-modal").data($(this).data());
	  $("p#remarks_lbl","div#approval-modal").hide();
	  $('textarea#obt-remarks',"div#approval-modal").val("").hide();
      $('div#approval-modal').modal('show');
	});
	
	$("#obt-reject").off("click").click(function(e){
	  e.preventDefault();
	  $('div#request-modal').modal('hide');
	  $('button.modal-action-btn',"div#approval-modal").data($(this).data());
	  $("p#remarks_lbl","div#approval-modal").show();
	  $('textarea#obt-remarks',"div#approval-modal").val("").show();
      $('div#approval-modal').modal('show');
	});
});