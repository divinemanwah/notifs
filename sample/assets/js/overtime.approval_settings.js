$(function () {
	...
	var group_table = $('table#group-codes-table').dataTable({
			serverSide: true,
			ajax: {
				url: base_url + 'overtime/getAllApprovalGroups/' + ($('input#app-grps-display-inactive').prop('checked') ? '1' : '0'),
				type: "POST"
			},
			deferRender: true,
			autoWidth: false,
			...
			rowCallback: function (r, d) {
			  $('a.group-edit', r).click(function (e) {
				e.preventDefault();
				$('button#apprv-grp-save',"div#apprv-grps-modal").data({id: d["ot_apprv_grp_id"]});
				$('div#apprv-grps-modal').modal('show');
			  });
			  $('a.group-remove', r).click(function (e) {
				e.preventDefault();
				$('button.delete-apprv-btn',"div#delete-modal").data({id: d["ot_apprv_grp_id"], label: d["ot_group_code"]});
	...
	$("input#app-grps-display-inactive").change(function(){
	  group_table_api.ajax.url(base_url + 'overtime/getAllApprovalGroups/' + ($('input#app-grps-display-inactive').prop('checked') ? '1' : '0')).load();
	});
	...
	.on('show.bs.modal', function () {
	  ...
	  $.getJSON(
		base_url + 'overtime/getApprovalGroup/' + $('button#apprv-grp-save').data('id'),
		function (response) {
		  $('input#apprv-grp-id'	,'div#apprv-grps-modal').val(response.data[0].ot_apprv_grp_id);
		  $('input#apprv-grp-code'	,'div#apprv-grps-modal').val(response.data[0].ot_group_code);
	...
	.on('show.bs.modal', function () {
	  ...
	  $.getJSON(
		base_url + 'overtime/getApprovalGroupFields',
		function (response) {
	...
	$("#apprv-grp-save", "#apprv-grps-modal").click(function(e) {
	  ...
	  $.post(
	    base_url + "overtime/updateApprovalGroup",
	...
	$("#apprv-grp-save-add", "#add-apprv-grps-modal").click(function(e) {
	  ...
	  $.post(
	    base_url + "overtime/insertApprovalGroup",
	...
	$(".delete-apprv-btn", "#delete-modal").click(function(e) {
	  e.preventDefault();
	  $.post(
	    base_url + "overtime/deleteApprovalGroup",
