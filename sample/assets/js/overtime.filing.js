$(function () {
  var req_schedules_tbl = $("#tk-ot-table");
  ...
  $('.input-date').datepicker({autoclose:true, format: "yyyy-mm-dd", startDate: yesterday});
  ...
  $("#requestForm").validate({
	...
	messages: {
			...
			"reason": {
				required: "Please specify the reason for filing Overtime"
			}
  ...
  $("#add-ot-btn").off("click").click(function(e) {
    e.preventDefault();
	$("input, textarea, select",'div#request-modal').removeAttr("disabled").val("");
	$("#ot-date-from").datepicker("setDate","").datepicker("update");
  ...
  $("#request-save").off("click").click(function(e){
	...
	$("#ot-sub-type","#requestForm").val($("#ot-type option:selected","#requestForm").attr("sub-id"));
	$.post(
	  base_url + "overtime/saveOTApplication",
  ...
  function getRequests(page_num) {
	...
	$("#tk-ot-loader").removeClass("hidden");
	$("#tk-ot-no-record").addClass("hidden");
	req_schedules_tbl.addClass("hidden");
	req_loading = true;
	current_page = page_num;
    $.ajax({
	  url: base_url+"overtime/getEmpOTGrid",
	  data: {limit: limit_default, page: page_num, dateFrom: $('#date-from').val(), dateTo: $('#date-to').val()},
	  cache: false,
	  dataType: "json",
	  type: "post",
	  success: function(response){
	    $("#tk-ot-loader").addClass("hidden");
	      ...
	      req_schedules_tbl.handsontable({
			...
			onSelection: function (r, c, r2, c2) {
			  if (this.view.settings.columns[c].readOnly) {
				setTimeout(function(){$("#tk-ot-no-record").handsontable('deselectCell');},100);
			  }
			}
		  });
		}
		else {
		  $("#tk-ot-no-record").removeClass("hidden");
		  req_schedules_tbl.addClass("hidden");
		}
		...
		pagination_lbl += '</ul>';
		$("#tk-ot-pager").html(pagination_lbl);
		req_loading = false;
	  }
	});
  }
  ...
  $(document).off("click",".request-view").on("click",".request-view",function(e) {
	...
	$.post(
	  ...
	  function(response){
		...
		$("#ot-date-from").val(response.data[0].date).attr("disabled",true);
		$("#ot-time-from").val(response.data[0].time_in).attr("disabled",true);
		$("#ot-time-to").val(response.data[0].time_out).attr("disabled",true);
  ...
  $(".modal-action-btn", "#action-modal").off("click").click(function(e) {
	...
	if($(this).data("action") == "delete") {
      $.post(
	    base_url + "overtime/deleteOTApplication",
      ...
      $.post(
	    base_url + "overtime/cancelOTApplication",
