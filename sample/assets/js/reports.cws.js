$(function () {
  var req_schedules_tbl = $("#tk-cws-table");
  ...
  $(document).off("click","div#tk-cws-pager ul.pagination li a").on("click","div#tk-cws-pager ul.pagination li a", function(e){
    e.preventDefault();
  ...
  function getRequests(page_num) {
	...
	$("#tk-cws-loader").removeClass("hidden");
	$("#tk-cws-no-record").addClass("hidden");
	req_schedules_tbl.addClass("hidden");
	req_loading = true;
	current_page = page_num;
    $.ajax({
	  url: base_url+"reports/getEmpCWSGrid",
	  ...
	  success: function(response){
		...
		if(response.total_count) {
		  ...
		  $("#tk-cws-no-record").removeClass("hidden");
		  req_schedules_tbl.addClass("hidden");
		}
		...
		pagination_lbl += '</ul>';
		$("#tk-cws-pager").html(pagination_lbl);
		req_loading = false;
	  }
	});
  }
