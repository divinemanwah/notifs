$(function () {
  ...
  $(document).on("click","div#tk-obt-pager ul.pagination li a", function(e){
    e.preventDefault();
  ...
  function getRequests(page_num) {
    ...
    $.ajax({
	  url: base_url+"reports/getEmpOBTGrid",
  ...
  $(document).off("click",".request-view").on("click",".request-view",function(e) {
	...
	$.post(
	  ...
	  function(response){
		...
		$("#obt-date").datepicker("setDate",response.data[0].date).attr("disabled",true);
