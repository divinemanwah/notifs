$(function () {
  ...
  $(document).on("click","div#tk-leave-pager ul.pagination li a", function(e){
    e.preventDefault();
  ...
  function getRequests(page_num) {
    ...
    $.ajax({
	  url: base_url+"reports/getEmpOvertimeGrid",
  ...
  $(document).off("click",".request-view").on("click",".request-view",function(e) {
	...
	$.post(
	  ...
	  function(response){
		...
		$("#ot-date-from").datepicker("setDate",response.data[0].date).attr("disabled",true);
