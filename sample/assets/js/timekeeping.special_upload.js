$(function () {
	  ...
	  serverSide: true,
	  ajax: {
		  url: base_url + 'timekeeping/getAllSpecialUploads',
		  type: "POST"
	  },
	  deferRender: true,
	  autoWidth: false,
		  ...
		  render: function (d,t,r,m) {
		    return '\
			  <div class="hidden-sm hidden-xs action-buttons">\
				...
				'+((r.status == 0)?'<a class="green upload-submit" href="" title="Upload"><i class="ace-icon fa fa-share-square-o bigger-130"></i></a>':'')+'\
				'+((r.status != 3)?'<a class="red upload-remove" href="#" title="Delete"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>':'')+'\
			  </div>\
			  <div class="hidden-md hidden-lg">\
				<div class="inline position-relative">\
				  ...
				  <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">\
					...
					'+((r.status==0)?'\
				    <li>\
					  <a href="#" class="tooltip-success upload-submit" data-rel="tooltip" title="Upload">\
					    <span class="green">\
  ...
  $("#upload-form","div.upload-file-widget").ajaxForm({
    url: base_url+"timekeeping/uploadSpecialSchedule",
	dataType: "json",
	type: "post",
	...
	success: function(response) {
	  if(response.success) {
		...
		uploads_table_api.ajax.url(base_url + 'timekeeping/getAllSpecialUploads').load();
  ...
  $(".modal-upload-btn", "#delete-modal").off("click").click(function(e) {
	...
	if($(this).data("action") == "delete") {
	  $.post(
	    base_url + "timekeeping/deleteSpecialSchedule",
	  ...
	  $.post(
	    base_url + "timekeeping/submitSpecialSchedule",
	    {upload_id: $('button.modal-upload-btn',"div#delete-modal").data("id")},
	    function(response) {
		  $('div#delete-modal').modal('hide'); 
		  uploads_table_api.ajax.url(base_url + 'timekeeping/getAllSpecialUploads').load();
