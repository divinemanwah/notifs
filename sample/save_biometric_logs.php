...
<?php
	//data from Biometric (POST all variables)
	$serverIP 		= $_POST['serverIP'];
	$sEnrollNumber 	= $_POST['sEnrollNumber'];
	$iAttState 		= $_POST['iAttState'];
	$sDate 			= $_POST['sDate'];
	$sHash 			= $_POST['sHash'];
	$iYear 			= $_POST['iYear'];
	$iMonth 		= $_POST['iMonth'];
	...
	$iSecond 		= $_POST['iSecond'];
	
	// if(!file_exists('C:\\biometrics\\Logs\\biometrics.txt'))
	// 	file_put_contents('C:\\biometrics\\Logs\\biometrics.txt', '');
	
	// file_put_contents('C:\\biometrics\\Logs\\biometrics.txt', $sEnrollNumber . ' : ' . $sDate . "\n", FILE_APPEND);
	...
	//Create hash in PHP code
	$compileLog = $sEnrollNumber . $iAttState . $sDate;
	$sHashPHP 	= hash('md5', $compileLog);
	...
	//Validate if records are valid
	If(($sHash == $sHashPHP) && ($serverIP == $ip_add))
		$valid_data = 1;
	Else
		$valid_data = 0;
	...
	if (mysqli_query($conn, $InsertData)) {
		...
		echo "Error: " . $InsertData . "<br>" . mysqli_error($conn);
			...
			echo "Record logs already exist. ";
		}
	...
	//file_put_contents('C:\xampp\htdocs\a.txt', $InsertData);
